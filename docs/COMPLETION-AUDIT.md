...
| Extension-author SDK for descriptor/tool packs | `docs/SDK.md`, `docs/TOOL-PACKS.md`, `src/tool_packs.rs`, `schemas/descriptor.schema.json`, `schemas/tool-pack.schema.json`, examples, `--validate-descriptor-dir`, `--validate-tool-pack-dir`, and `--list-tool-packs`; CLI tests and validation passed. |
...
- Descriptor parser backends now have explicit validation, `--list-backends` discovery, and core `ParseOptions` backend selection. `heuristic`, `sections`, `tree-sitter`, `tree-sitter-rust`, `executable-json`, and `wasm-json` are implemented. `tree-sitter` currently requires `parser.grammar = "rust"` and supports descriptor-local query metadata. `wasm-json` loads descriptor-declared WebAssembly parser modules with fuel metering and a versioned JSON envelope.
