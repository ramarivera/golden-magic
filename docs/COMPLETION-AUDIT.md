...
- Descriptor parser backends now have explicit validation, `--list-backends` discovery, and core `ParseOptions` backend selection. `heuristic`, `sections`, `tree-sitter`, `tree-sitter-rust`, and `executable-json` are implemented. `tree-sitter` currently requires `parser.grammar = "rust"` and supports descriptor-local query metadata.
