...
- Descriptor parser backends now have explicit validation, `--list-backends` discovery, and core `ParseOptions` backend selection. `heuristic` is implemented; `tree-sitter` remains a planned candidate and fails validation until implemented.
